...
import { useReportsDeductedModelsQuery } from '../../../../query/statistics/reportsDeductedModels.query'
import AppError from '../../../../components/AppError'
import AppDataTable from '../../../../components/AppDataTable'
import type { ColumnDef, SortingState, PaginationState } from '@tanstack/react-table'
import type { TReportsDeductedModel } from '../../../../types'
...
function DeductedModelsPage() {
  ...
  const { data: deductedModels, isLoading, isError, error } = useReportsDeductedModelsQuery({ year: queryYear, month: queryMonth, page, pageSize: pagination.pageSize })

  const rows = useMemo<TReportsDeductedModel[]>(() => {
    const items = Array.isArray(deductedModels?.data) ? deductedModels.data : (Array.isArray(deductedModels) ? deductedModels : [])
  ...
  const columns = useMemo<ColumnDef<TReportsDeductedModel>[]>(
    () => [
      {
      ...
      },
      {
        ...
        header: t('deletedDate', { ns: 'newModels' }),
        enableSorting: true,
        cell: ({ row }) => dateTimeFormat(row.original.deletedDate),
      },
    ],
    [t, dateTimeFormat]
  )
    ...
    <Box>
      ...
      <StyledPaper sx={{ mt: 3 }}>
        {isError ? (
          ...
          <AppDataTable<TReportsDeductedModel>
            tableName="deductedModels"
            data={rows}
            columns={columns}
            isLoading={isLoading}
            ...
            totalPages={deductedModels?.totalPagesNumber ?? 1}
            currentPage={deductedModels?.currentPageNumber ?? pagination.pageIndex + 1}
