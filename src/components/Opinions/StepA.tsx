...
import type { TCarType, TCustomer, TDriveType, TExternalStatus, TGearbox, TImporter, TInternalStatus, TOpinion, TSelectedOwner, TUsageType } from "../../types"
...
export default function StepA({ opinion, onStepComplete, setIsTemporary }: TProps) {
  ...
  const { isLoading: isIntegralExtrasLoading } = useIntegralExtrasQuery()
  const { isLoading: isExtrasLoading } = useExtrasQuery()
  const { isLoading: isProtectivesLoading } = useProtectivesQuery()
  ...
  useEffect(() => {
    const result: TSelectedOwner[] = selectedOwners
      .filter((item: TSelectedOwner) => !!item.ownerId)
      .map((item: TSelectedOwner) => ({
      ...
      <Grid size={12} sx={{ display: 'flex', gap: 2, justifyContent: 'space-between' }}>
        <Box>
          <Typography sx={{ fontWeight: 'bold' }} variant="body1" >
            {opinion?.number ?? t('new', { ns: 'opinion' })}
          </Typography>
        </Box>
