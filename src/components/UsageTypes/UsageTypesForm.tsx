...
interface Props {
  data: TUsageType | null
  isPending: boolean
  onCancel: () => void
  onConfirm: (data: TUsageType) => void
}

type TFormInput = Omit<TUsageType, 'id'>

function UsageTypesForm({ data, isPending, onCancel, onConfirm }: Props) {
      ...
      <Grid container sx={{ mt: 1 }}>
        <Grid size={12}>
          <AppControlledTextField
            ...
            control={control}
            errors={errors}
            label={t('name', { ns: 'usageTypes' })}
            placeholder={t('name', { ns: 'usageTypes' })}
          />
        </Grid>
        <Grid size={12}>
          <AppControlledTextField
            ...
            control={control}
            errors={errors}
            label={t('nameEn', { ns: 'usageTypes' })}
            placeholder={t('nameEn', { ns: 'usageTypes' })}
          />
        </Grid>
