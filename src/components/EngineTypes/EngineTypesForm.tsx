...
interface Props {
  data: TEngineType | null
  isPending: boolean
  onCancel: () => void
  onConfirm: (data: TEngineType) => void
}

type TFormInput = Omit<TEngineType, 'id'>

function EngineTypesForm({ data, isPending, onCancel, onConfirm }: Props) {
  ...
  const schema = object()
    .shape({
      ...
      priceListBackground: yup.string(),
    })
    .required()
  ...
  const methods = useForm<TFormInput>({
    ...
    defaultValues: {
      ...
      nameEn: data?.nameEn || '',
      code: data?.code || 0,
      priceListBackground: data?.priceListBackground || '#cccccc',
    },
  })
...
export default EngineTypesForm